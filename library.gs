...
function logGame(sheet, playerNo, playerObj, setOfVals) {
  let playerCols = ['A', 'M', 'Y', 'AK'];
  let cols = ['info', 'stats', 'calmness', 'stress', 
  'hindrance', 'hindrance-2', 'hindrance-3', 'hindrance-4', 'hindrance-5',
  'poison', 'poison-2', 'poison-3'];

  let newRow   = sheet.getLastRow() + 1;

  _.each(setOfVals, function(v, k, coll){
    if(_.indexOf(['info', 'stats', 'calmness', 'stress'], k)>-1){
      let loc = fromA1Notation(playerCols[playerNo]+(_.indexOf(cols, k)+1));
      sheet.getRange(newRow, loc.column+_.indexOf(['info', 'stats', 'calmness', 'stress'], k)).setValue(v);
    }
    if('hindrance'==k){
      let loc = fromA1Notation(playerCols[playerNo]+'1');
      sheet.getRange(newRow, loc.column + 8).setValue((isFlagSet(v, 16) ? 'X' : ''));
      sheet.getRange(newRow, loc.column + 7).setValue((isFlagSet(v, 8) ? 'X' : ''));
      sheet.getRange(newRow, loc.column + 6).setValue((isFlagSet(v, 4) ? 'X' : ''));
      sheet.getRange(newRow, loc.column + 5).setValue((isFlagSet(v, 2) ? 'X' : ''));
      sheet.getRange(newRow, loc.column + 4).setValue((isFlagSet(v, 1) ? 'X' : ''));
    }
    if('poison'==k){
      let loc = fromA1Notation(playerCols[playerNo]+'1');
      sheet.getRange(newRow, loc.column + 11).setValue((isFlagSet(v, 4) ? 'X' : ''));
      sheet.getRange(newRow, loc.column + 10).setValue((isFlagSet(v, 2) ? 'X' : ''));
      sheet.getRange(newRow, loc.column +  9).setValue((isFlagSet(v, 1) ? 'X' : ''));
    }
  });
